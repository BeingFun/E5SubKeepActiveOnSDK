...
if __name__ == '__main__':
    threewords_path = ROOT_PATH + r'\src\\E5_Subscription.py'
    performable_path = ROOT_PATH + r'\bin'
    icon_file_path = ROOT_PATH + r'\resources\\ico\\E5_Subscription.ico'
    work_path = CUR_PATH + r'\E5_Subscription_build'
    # 创建运行时需要的文件夹
    new_folder = [ROOT_PATH + r'\tmp_file', ROOT_PATH + r'\logs']
    ...
    options = [
        ...
        # '--noconsole',  # 不显示控制台窗口
        '--clean',  # 清理临时文件
        '--name={}'.format("E5_Subscription"),  # 指定生成的可执行文件名称
